...
export default function DiagnosticReport({
  ...
  return (
    <>
      <AppHeader />
      <div className="min-h-screen bg-gradient-to-br from-[#0093c9]/10 via-white to-[#ea9c48]/10 py-8 pt-24">
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center space-x-2 px-4 py-2 rounded-lg font-semibold transition-all duration-200 ${activeTab === tab.id
                          ...
                          {[
                            ...
                            .map((strength, index) => (
                              <div key={strength.name} className="flex items-center space-x-2 text-sm">
                                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-white text-xs font-bold ${index === 0 ? 'bg-yellow-500' : index === 1 ? 'bg-gray-400' : 'bg-orange-500'
                                  }`}>
                                  {index + 1}
                                </div>
